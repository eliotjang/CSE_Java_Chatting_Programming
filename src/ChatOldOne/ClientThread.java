...
public class ClientThread extends Thread
{
   ...
   private static final int REQ_WISPERSEND = 1022; //추가
   private static final int REQ_LOGOUT = 1031;
   private static final int REQ_CREATROOM = 1040;
   ...
   private static final int YES_CREATEROOM = 2014; //추가
	private static final int NO_CREATEROOM = 2015; //추가
   private static final int MDY_ROOM = 2017; //추가
   ...
   private static final int C_LOGOUT = 2032;
         ...
         while(currThread == thisThread){ // 종료는 LOG_OFF에서 thisThread=null;에 의하여
            ...
            switch(command){
               ...
               case YES_LOGON:{
                  ...
                  if(rooms.compareTo("0")!=0) {
                	  users = new StringTokenizer(rooms, DELIMETER);
               ...
               case YES_CREATEROOM:{
            	   ...
            	   while(roomusers.hasMoreTokens()) {
            		   ct_client.cc_lstRoom.add(roomusers.nextToken());
            	   }
            	   break;
               }
               case NO_CREATEROOM:{
...
public class ClientThread extends Thread {
			...
			while (currThread == thisThread) { // 종료는 LOG_OFF에서 thisThread=null;에 의하여
				...
				case YES_QUITROOM: {
					...
					if(rooms.compareTo(" ")==0) {
						...
						StringTokenizer Rooms = new StringTokenizer(rooms, DELIMETER);
						ct_client.cc_lstRoom.removeAll();
						while (Rooms.hasMoreTokens()) {
							ct_client.cc_lstRoom.add(Rooms.nextToken());
						}
					}
	...
	// EnterRoom 패킷(REQ_ENTERROOM|ID)을 생성하고 전송한다.
	public void requestEnterRoom(String id,String Room) {
		try {
			ct_buffer.setLength(0); // EnterRoom 패킷을 생성한다.
			ct_buffer.append(REQ_ENTERROOM);
			ct_buffer.append(SEPARATOR);
	...
	public void requestSendWords(String words) {
		try {
			...
			ct_buffer.append(words);
			ct_buffer.append(SEPARATOR);
			ct_buffer.append(ct_client.cc_lstRoom.getSelectedItem());
			send(ct_buffer.toString()); // SendWords 패킷을 전송한다.
		} catch (IOException e) {
			System.out.println(e);
		}
