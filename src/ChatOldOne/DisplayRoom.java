...
public class DisplayRoom extends Frame implements ActionListener, KeyListener {
	...
	public DisplayRoom(ClientThread client, String title) {
		...
		dr_btwisper = new Button("쪽지하기");
		dr_btwisper.addActionListener(this);
		northpanel.add(dr_btwisper);
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (b.getLabel().equals("화면지우기")) {
			...
			dr_thread.requestQuitRoom();
			// 로그아웃 처리 루틴
		}else if(b.getLabel().equals("쪽지하기")) {
			...
			if (item != null) {
				...
				dr_thread.requestWisperSend(words, item);
				 ...
				 MessageBox msgBox = new MessageBox(this, "귓속말", "귓속말할 대상을 선택 후 내용을 적고 버튼을 눌러주세요.");
	             msgBox.show();
			}
		}
	}
