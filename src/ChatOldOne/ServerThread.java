...
public class ServerThread extends Thread 
{
   ...
   private static final int MDY_LOGONUSERS = 2014;
   private static final int MDY_ROOMUSERS = 2015;
         ...
         while(true){
            ...
            switch(command){
               ...
               case REQ_ENTERROOM:{
                  ...
                  st_buffer.append(userIDs);
                  broadcast(st_buffer.toString()); // MDY_USERIDS 패킷을 전송한다. → MDY_ROOMUSERS 패킷 전송
                  break;
               }
   ...
   /* 해쉬 테이블에 접속을 요청한 클라이언트의 ID 및 전송을 담당하는 스레드를 등록.
    ...
    private static synchronized int delUser(String id){
        if(checkUserID(id) != null){
        	roomVector.removeElement(id);  // 사용자 ID 추가
            roomHash.remove(id); // 사용자 ID 및 클라이언트와 통신할 스레드를 저장한다.
        }  
        if(logonHash.size() >= ChatServer.cs_maxclient){
           return MSG_SERVERFULL;
        }
